...
// now the windows version of bitcoind sees all the cleanup code run in the 
// destructors of CNetCleanup in net.cpp and 
// CMainCleanup in main.cpp (well not in this version, but later versions!)

static int WindowsHandleSigterm(unsigned long the_signal )
{
bool
    I_will_handle_it = false;   // we get to choose who handles the signal

switch( the_signal )            // we call the current HandleSIGTERM() here
    {                           // this is what wasn't happening in Windows!

    case CTRL_C_EVENT:                    //A CTRL+c signal was received,
        HandleSIGTERM( (int)the_signal ); //either from keyboard input      
        I_will_handle_it = true;          //or from a signal generated by the   
        break;                            //GenerateConsoleCtrlEvent function    

    case CTRL_BREAK_EVENT:    //A CTRL+BREAK signal was received, either from 
        // keyboard input or from a signal generated by GenerateConsoleCtrlEvent. 
        break;

    case CTRL_CLOSE_EVENT:    //A signal that the system sends to all processes 
        // attached to a console when the user closes the console 
        // (either by choosing the Close command from the console window's 
        // System menu, or by choosing the End Task command from the Task List). 
        break;

    case CTRL_LOGOFF_EVENT:   //A signal that the system sends to all console 
        // processes when a user is logging off. This signal does not indicate 
        // which user is logging off, so no assumptions can be made. 
        break;

    case CTRL_SHUTDOWN_EVENT: // I think we should do something here too?   
        HandleSIGTERM( (int)the_signal );   // why do it here too. It seems
        I_will_handle_it = true;            // reasonable!
        break;
    default:
        break;    
    }
return I_will_handle_it;
}
#endif
...
#else
    // since I don't know if MAC is an ifndef WIN32 or not
    // let's be explicit here for windows
        
    // the later versions 0.8.3, 0.8.4 of bitcoin-qt also fail on
    // exit or stop, and it seems that the "quit" code never hits
    // but I haven't found why? Qt 0.8.1 exits quietly though.

    #ifdef WIN32
    // what do we do for windows aborts or Ctl-Cs, etc.?
    // we make our own HandleSigTerm!
    if( SetConsoleCtrlHandler( (PHANDLER_ROUTINE)&WindowsHandleSigterm, true ) )
        ;   // success! It does have a return value, which we will ignore!
  //else                         exigency of wincon.h
  //    // we failed! So we are just back where we were before, with no code.
    ...
    //
    // func can be a function or a constant SIG_DFL or SIG_IGN. 
    //
    #endif
